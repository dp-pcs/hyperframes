#!/usr/bin/env bash
# deploy-amplify-worker.sh — build, push, and roll out the amplify
# video worker to amplify-dev ECS.
#
# Usage:
#   ./scripts/deploy-amplify-worker.sh                  # full deploy
#   SKIP_DEPLOY=1 ./scripts/deploy-amplify-worker.sh    # build + push only
#   SKIP_WAIT=1 ./scripts/deploy-amplify-worker.sh      # don't wait for stable
#
...
ECR_REPO="${ECR_REPO:-$AWS_ACCOUNT.dkr.ecr.$AWS_REGION.amazonaws.com/amplify/dev/video-worker}"
IMAGE_TAG="${IMAGE_TAG:-$(git rev-parse --short HEAD)}"
ECS_CLUSTER="${ECS_CLUSTER:-amplify-dev-cluster}"
ECS_SERVICE="${ECS_SERVICE:-amplify-dev-video-worker}"
