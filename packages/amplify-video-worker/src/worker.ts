...
const queueUrl = process.env.AMPLIFY_VIDEO_QUEUE_URL?.trim() || "";
...
function buildExplainerScript(args: {
  ...
  const publicationName = args.brief.article.publication?.name || "Amplify";
  ...
  <head>
    ...
    <style>
      ...
      * {
      ...
      [data-composition-id="amplify-explainer"] {
        position: relative;
        width: ${dims.width}px;
        height: ${dims.height}px;
        overflow: hidden;
...
const SKILLS_ROOT =
  process.env.AMPLIFY_WORKER_SKILLS_ROOT?.trim() || joinPath(__amp_dirname, "..", "skills");
...
const COMPOSITION_MAX_ATTEMPTS = Number.parseInt(
  process.env.AMPLIFY_COMPOSITION_MAX_ATTEMPTS || "5",
  10,
);
const COMPOSITION_LLM_TIMEOUT_MS = Number.parseInt(
  process.env.AMPLIFY_COMPOSITION_LLM_TIMEOUT_MS || "240000",
  10,
);
...
async function authorNarrationPlan(opts: {
  ...
  try {
    ...
    const result = await authorComposition<{
      ...
      schemaName: "amplify_explainer_narration",
      ai: opts.ai,
...
async function processRenderJob(message: AmplifyQueueMessage) {
  ...
  try {
    ...
    workDir = mkdtempSync(join(tmpdir(), "amplify-video-worker-"));
    ...
    const loopResult = await runCompositionLoop({
      ...
      llm: async (conversation) => {
        ...
        return authorComposition({
          ...
          schemaName: "amplify_explainer_composition",
          ai: {
            baseUrl: user.aiBaseUrl!,
            apiKey: user.aiApiKey!,
            model: user.aiModel!,
    ...
    if (!usingFallback && authoredIndexHtml) {
      ...
      // amplify-constraints.md) but ElevenLabs narration is unbounded.
...
async function pollOnce() {
  ...
  let parsed: AmplifyQueueMessage;
  try {
    parsed = JSON.parse(message.Body) as AmplifyQueueMessage;
  } catch (error) {
  ...
  try {
    ...
    failedJobs += 1;
    console.error("Failed to process Amplify explainer job", {
      jobId: parsed.jobId,
...
async function runLoop() {
  if (!queueUrl) {
    throw new Error("Missing AMPLIFY_VIDEO_QUEUE_URL");
  }
...
export function startHealthServer() {
  ...
  server.listen(healthPort, () => {
    console.log(`Amplify video worker health server listening on ${healthPort}`);
  });
  return server;
}
...
export type AmplifyWorkerHealthServer = Server;
